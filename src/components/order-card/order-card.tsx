...
const getStatusColor = (status: Status): string => {
  switch (status) {
    case "Đã hủy":
      return "text-red-500 border-[1.5px] border-red-500 ";
    case "Đang tiến hành":
      return "text-yellow-500 border-[1.5px] border-yelloe-500 ";
    case "Hoàn thành":
      return "text-green-500 border-[1.5px] border-green-500 ";
    case "Đã giao":
      return "text-blue-500 border-[1.5px] border-blue-500 ";
    default:
      return "text-black border-[1.5px] border-black-500 "; // Màu mặc định nếu trạng thái không khớp
  }
};
...
const OrderCard: FC<DetailOrderAdmin> = ({OrderID, timeOrder, DetailOrders, status}) => {
    return (
        <div>
            <div className='containerDetailcard'>
                ...
                <div >
                <span className="block font-display text-[15px] font-semibold text-[#797B7E]">
                    {DetailOrders.length === 0 ? `x0 items` : `x${DetailOrders.length} items`}
                    </span>{" "}
                </div>
                <div>
                    <label htmlFor="" className={`${getStatusColor(status)} rounded-md p-1`}>
                        {status}
                    </label>
                </div>
                </div>
